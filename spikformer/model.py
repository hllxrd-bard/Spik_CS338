...
def cfg_get(cfg: Any, key: str, default: Any = None) -> Any:
    """
    Read config from dict / OmegaConf-like object / argparse Namespace.
    """
    if cfg is None:
        return default
...
def build_spike_node(
    neuron_cfg: Any = None,
    *,
    v_threshold: Optional[float] = None,
) -> nn.Module:
    """
    Build LIF / PLIF node from config.
    ...
    Expected config fields:
        ...
        v_reset: float | None
    """
    neuron_type = str(cfg_get(neuron_cfg, "type", "lif")).lower()

    tau = float(cfg_get(neuron_cfg, "tau", 2.0))
    detach_reset = bool(cfg_get(neuron_cfg, "detach_reset", True))
    backend = str(cfg_get(neuron_cfg, "backend", "cupy"))

    cfg_v_threshold = float(cfg_get(neuron_cfg, "v_threshold", 1.0))
    final_v_threshold = cfg_v_threshold if v_threshold is None else float(v_threshold)

    v_reset = cfg_get(neuron_cfg, "v_reset", 0.0)
    if isinstance(v_reset, str) and v_reset.lower() in {"none", "null"}:
        v_reset = None

    if neuron_type == "lif":
        return MultiStepLIFNode(
            tau=tau,
            v_threshold=final_v_threshold,
            v_reset=v_reset,
    ...
    if neuron_type == "plif":
        if MultiStepParametricLIFNode is None:
            raise ImportError(
                "MultiStepParametricLIFNode is not available in your SpikingJelly version. "
                "Either install a compatible clock_driven version or migrate to "
    ...
    raise ValueError(f"Unknown neuron type: {neuron_type}. Expected 'lif' or 'plif'.")
...
class SpikingMLP(nn.Module):
    def __init__(
        ...
        self.fc1_lif = build_spike_node(neuron_cfg)

        self.fc2_conv = nn.Conv1d(hidden_features, out_features, kernel_size=1, stride=1)
        self.fc2_bn = nn.BatchNorm1d(out_features)
        self.fc2_lif = build_spike_node(neuron_cfg)
...
class SpikingSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_lif = build_spike_node(neuron_cfg)

        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = build_spike_node(neuron_cfg)

        self.v_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = build_spike_node(neuron_cfg)

        self.attn_lif = build_spike_node(
            neuron_cfg,
            v_threshold=0.5,
        )
        ...
        self.proj_lif = build_spike_node(neuron_cfg)

    def _project(self, x: torch.Tensor, conv: nn.Conv1d, bn: nn.BatchNorm1d, lif: nn.Module) -> torch.Tensor:
        t, b, c, n = x.shape
...
class SpikingConvPoolStage(nn.Module):
    def __init__(
        ...
        self.lif = build_spike_node(neuron_cfg)
...
class SpikformerFeatureStage(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.rpe = SpikeRelativePositionEmbedding(
            channels,
            neuron_cfg=neuron_cfg,
        )
...
class MultiscaleSpikformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.sps1 = SpikingConvPoolStage(in_channels, c1, neuron_cfg=neuron_cfg)
        self.sps2 = SpikingConvPoolStage(c1, c2, neuron_cfg=neuron_cfg)
        self.sps3 = SpikingConvPoolStage(c2, c3, neuron_cfg=neuron_cfg)
        self.sps4 = SpikingConvPoolStage(c3, c4, neuron_cfg=neuron_cfg)
        self.sps5 = SpikingConvPoolStage(c4, c5, neuron_cfg=neuron_cfg)

        self.stage3 = SpikformerFeatureStage(
            c3,
            stage_heads[0],
            stage_depths[0],
            mlp_ratio,
        ...
        self.stage4 = SpikformerFeatureStage(
            c4,
            stage_heads[1],
            stage_depths[1],
            mlp_ratio,
        ...
        self.stage5 = SpikformerFeatureStage(
            c5,
            stage_heads[2],
            stage_depths[2],
            mlp_ratio,
