...
from yolox.network_blocks import BaseConv, DWConv
...
class YOLOXHead(nn.Module):
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        # Important: create grid on the same device as output.
        if (
            grid.ndim < 4
            or grid.shape[2:4] != output.shape[2:4]
            or grid.device != output.device
            ...
            grid = grid.to(device=output.device, dtype=output.dtype)
            self.grids[k] = grid
    ...
    def decode_outputs(self, outputs):
        need_new_grid = (
            self.output_grids is None
            or self.output_strides is None
            or self.output_grids.device != outputs.device
            or self.output_grids.dtype != outputs.dtype
            or self.output_grids.shape[1] != outputs.shape[1]
        )
    ...
    def get_assignments(
        ...
        if mode == "cpu":
            target_device = cls_preds.device
